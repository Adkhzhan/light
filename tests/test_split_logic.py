...
def test_expense_analysis_uses_payer_named_in_description(monkeypatch):
    monkeypatch.setattr(
        app_module,
        "classify_expense_with_nemotron",
        lambda expense_text, group, payer: {
            ...
            "category": "food",
            "amount_confidence": "high",
            "participants_included": ["Sam", "Priya", "Leo"],
            "participants_excluded": ["Alex"],
            "split_hint": "exclude_named",
            "ambiguity_flags": [],
        },
    )
    ...
    response = app.test_client().post(
        "/expense-analysis?text=Sam%20paid%20%2490%20for%20dinner.%20Alex%20wasn%27t%20there."
    )
